...
- [layout.ts#L3-L10](file:///c:/Users/LENOVO/Desktop/Chnk it/src/config/layout.ts#L3-L10) — file header comment says `SNAP_STEP is 112px (2 * BASE_UNIT)` but line 10 sets `export const SNAP_STEP = 56; // Unified to 56 for consistency`.
- [CanvasBoard.tsx#L150](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/canvas/CanvasBoard.tsx#L150) — `const snapGrid: [number, number] = [56, 56];` (hard-coded, not sourced from `layout.ts`).
- [useCanvasNodeDrag.ts](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/canvas/hooks/useCanvasNodeDrag.ts) lines 184-185 — uses `snapToGridValue` which rounds to `BASE_UNIT` (56px).
- Only the **fused note** flow uses the 2-unit step via `MODULE_SNAP_STEP = 112` ([layout.ts#L27](file:///c:/Users/LENOVO/Desktop/Chnk it/src/config/layout.ts#L27)).
...
[StorageManager.ts `autoReconnect`](file:///c:/Users/LENOVO/Desktop/Chnk it/src/services/StorageManager.ts) catches everything and returns without surfacing a reason. If permission was revoked or the folder moved, the user sees the "disconnected" icon with no diagnostic.
...
- [CoverPicker.tsx#L1](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/card/CoverPicker.tsx#L1) — unused `useEffect` import.
- [NoteCard.tsx#L577](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/card/NoteCard.tsx#L577) — `NoteExpandedContent` is passed `showMetadata` prop that does not exist on the component's props type (`TS2322`). **This is a real bug**, not lint — the prop is being dropped silently.
- [ColorBlock.tsx#L199](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/editor/ColorBlock.tsx#L199) — unused `autoName`.
- [ContainerBlock.tsx#L1](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/editor/ContainerBlock.tsx#L1) — unused `React` import.
- [useBlockCommands.ts#L4, L27](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/editor/hooks/useBlockCommands.ts) — unused `useStore` import and `nodeId` parameter.
- [VirtualBlockList.tsx#L3](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/editor/VirtualBlockList.tsx#L3) — `react-window` no longer exports `FixedSizeList` / `ListChildComponentProps` (API was renamed to `CellComponentProps`). **This file currently does not compile.** Virtual block rendering is broken.
- [KanbanCalendarView.tsx#L80](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/kanban/KanbanCalendarView.tsx#L80) — unused `getDaysInMonth`.
- [KanbanColumn.tsx#L46](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/kanban/KanbanColumn.tsx#L46) — unused callback param.
- [SearchResults.tsx#L4](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/ui/SearchResults.tsx#L4), [searchUtils.ts#L1](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/ui/searchUtils.ts#L1) — unused imports.
- [FileSystemStorage.ts#L41, L237, L489](file:///c:/Users/LENOVO/Desktop/Chnk it/src/services/FileSystemStorage.ts) — unused private field `_lastSavedState`, unused `options`, dead `syncCardsToFolders` (commented out at call site).
- [nodeSlice.ts#L577](file:///c:/Users/LENOVO/Desktop/Chnk it/src/store/slices/nodeSlice.ts#L577) — unused `currentParentId`.
- [throttle.ts#L18, L26](file:///c:/Users/LENOVO/Desktop/Chnk it/src/utils/throttle.ts) — `TS2683: 'this' implicitly has type 'any'`.
...
[main.tsx#L15-L19](file:///c:/Users/LENOVO/Desktop/Chnk it/src/main.tsx) and [App.tsx#L7-L11](file:///c:/Users/LENOVO/Desktop/Chnk it/src/App.tsx) both render `<ErrorBoundary>`. Harmless but redundant.
...
**Observed:** matches. [CanvasBoard.tsx#L195-L198](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/canvas/CanvasBoard.tsx#L195-L198) sets `minZoom={0.05}`, `maxZoom={2}`, `snapToGrid={true}`. Custom grid in `DragGridOverlay.tsx` + `CustomGrid.tsx`. Viewport culling in `useCanvasViewport.ts`.
...
**Observed:**
- `findNonOverlappingPosition` lives in [BottomMenu.tsx#L86-L177](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/ui/BottomMenu.tsx#L86-L177) and is re-implemented inline. **Not grid-snapped** — it returns raw candidate coords without rounding to `SNAP_STEP` or `MODULE_SNAP_STEP`. Combined with C1, creation can place a node off-grid until the first drag snaps it.
- Default new-note size is hard-coded `432 x 432` ([BottomMenu.tsx#L184-L185](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/ui/BottomMenu.tsx#L184-L185)) — equal to `MIN_EXPANDED_SIZE` in `layout.ts` but duplicated.
...
**Finding:** multi-node drag offset math in [useCanvasNodeDrag.ts#L182-L187](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/canvas/hooks/useCanvasNodeDrag.ts#L182-L187) looks correct for the 56px choice. If/when the product picks 112px, this hook must also be updated.
...
**Observed:** [calculateNoteLayout in layout.ts#L61-L100](file:///c:/Users/LENOVO/Desktop/Chnk it/src/config/layout.ts#L61-L100) matches the spec thresholds. Auto-grow present on NoteCard (tracked via `activeResize` ref and ResizeObserver).
...
**Observed:** [nodeSlice onConnect](file:///c:/Users/LENOVO/Desktop/Chnk it/src/store/slices/nodeSlice.ts) present. No observed defect in a quick read — needs runtime exercise to confirm the `parentId` field on edges is actually populated as the story claims.

### Story 6 — Box Selection & Multi-Node Operations (no defects found)

**Expected:** Ctrl-drag box selection, bulk operations.
**Observed:** [useCanvasBoxSelection.ts](file:///c:/Users/LENOVO/Desktop/Chnk it/src/features/canvas/hooks/useCanvasBoxSelection.ts) implements it; `selectionMode={SelectionMode.Partial}`; `nodeSlice` exposes `bulkDeleteNodes`, `bulkDuplicateNodes`, `bulkApplyColor`.
...
**Observed:** [navigationSlice.ts#L5](file:///c:/Users/LENOVO/Desktop/Chnk it/src/store/slices/navigationSlice.ts#L5) reads `localStorage.getItem('chnk-it-current-parent-id')` **synchronously at slice creation**. This fires before `storage.loadGraph` hydrates the nodes from disk/cloud, meaning on cold start `currentParentId` points to a node that does not yet exist.
...
**Observed:** [navigationSlice.ts#L47-L51](file:///c:/Users/LENOVO/Desktop/Chnk it/src/store/slices/navigationSlice.ts#L47-L51) — each setter nulls the others, enforcing mutual exclusion. Good.
...
6. **Story 10 fix:** clear `chnk-it-current-parent-id` in `localStorage` when reconstruction falls back to Home *after* the first successful load.
