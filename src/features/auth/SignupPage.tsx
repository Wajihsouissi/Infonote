...
export const SignupPage: React.FC = () => {
  ...
  const handleOAuth = async (provider: 'google' | 'figma') => {
    setError(null);
    if (!isSupabaseConfigured || !supabase) {
      setError('Authentication is not configured. Please contact the administrator (missing VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY).');
      return;
    ...
    <div className={styles.shell}>
      ...
      <div className={styles.leftPanel}>
        ...
        <div className={styles.leftHeader}>
          {hasEnteredApp && (
            <Button variant="secondary" size="sm" className={styles.backButton} onClick={() => setCurrentView('landing')} leadingIcon={<ArrowLeft size={14} />}>
              Back to home
            </Button>
          )}
      ...
      <div className={styles.rightPanel}>
        <div className={styles.formContainer}>
          ...
          <div className={styles.socialGroup}>
            <Button variant="secondary" className={styles.socialButton} onClick={() => handleOAuth('google')} disabled={loading} leadingIcon={
            ...
            <Button variant="secondary" className={styles.socialButton} onClick={() => handleOAuth('figma')} disabled={loading} leadingIcon={
            ...
            <Button variant="secondary" className={styles.socialButton} onClick={async () => { setError(null); setLoading(true); const result = await connectNotion(); if (!result.ok) { setError(result.error); } setLoading(false); }} disabled={loading} leadingIcon={
          ...
          <form className={styles.form} onSubmit={handleSignup}>
            ...
            <Button type="submit" variant="primary" fullWidth className={styles.submitButton} loading={loading} disabled={!email || !password || !firstName || !lastName || !confirmPassword} leadingIcon={<UserPlus size={15} />}>
              Create account
            </Button>
